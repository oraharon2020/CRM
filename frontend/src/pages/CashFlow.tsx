      ...
      try {
        ...
        try {
          ...
          if (response && response.success && response.data && response.data.length > 0) {
            ...
            // For other sources, adjust the data to match the total revenue from storesAPI
            ...
            if (totalRevenue > 0 && Math.abs(totalRevenue - currentTotal) > 1) {
              ...
              if (daysWithRevenue.length > 0) {
                ...
                // If all days have zero revenue, distribute evenly
                const daysInMonth = filteredData.length;
                const revenuePerDay = totalRevenue / daysInMonth;
                
                filteredData = filteredData.map((item: CashFlowData) => ({
                  ...item,
                  revenue: revenuePerDay,
                  orderCount: Math.round(totalOrders / daysInMonth),
                  productCount: Math.round(totalProducts / daysInMonth)
                }));
                
                console.log('Distributed revenue evenly across all days');
              }
            }
