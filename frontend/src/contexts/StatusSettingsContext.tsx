...
import { StatusSettings } from '../types/settings.types';
...
interface StatusSettingsContextType {
  ...
  isLoading: boolean;
  error: string | null;
  updateStatusSettings: (storeId: number, statuses: string[]) => Promise<void>;
...
export const StatusSettingsProvider: React.FC<StatusSettingsProviderProps> = ({ children }) => {
  ...
  const updateStatusSettings = async (storeId: number, statuses: string[]) => {
    try {
      setError(null);
