...
const StatusSettingsModal: React.FC<StatusSettingsModalProps> = ({
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          if (statusSettings.included_statuses[storeId]) {
            setSelectedStatuses(statusSettings.included_statuses[storeId]);
          } else {
            // Default to all statuses if none are selected
            setSelectedStatuses(allStatuses.map(status => status.value));
          }
        } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateStatusSettings(storeId, selectedStatuses);
      onSave();
      onClose();
    } catch (error) {
