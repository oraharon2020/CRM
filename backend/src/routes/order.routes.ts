...
router.get('/', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (statuses && statuses.length > 0) {
      ...
      wooParams.status = statuses;
      console.log('Using multiple statuses filter:', statuses);
  ...
  try {
    ...
    try {
      ...
      const order = {
        ...
        date: wooOrder.date_created.split('T')[0],
        ...
        history: [
          { date: wooOrder.date_created.replace('T', ' ').substring(0, 19), status: 'התקבלה', user: 'מערכת' },
